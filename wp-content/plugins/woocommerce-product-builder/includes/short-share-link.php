...
class VI_WPRODUCTBUILDER_Short_Share_Link {
	...
	public function save_short_link_edit() {
		if ( current_user_can( 'manage_options' ) && isset( $_POST['_nonce'] ) && wp_verify_nonce( $_POST['_nonce'], '_woopb_add_to_woocommerce' ) && ! empty( $_POST['woopb_save_edit_short_link'] ) && ! empty( $_POST['woopb_id'] ) ) {
	...
	public function get_short_share_link() {
		if ( ! ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], '_woopb_add_to_woocommerce' ) ) ) {
			return;
		}

		$page_id = isset( $_POST['woopb_id'] ) ? sanitize_text_field( $_POST['woopb_id'] ) : '';
		if ( ! $page_id ) {
			return;
		}
...
//		wc()->session->__unset( 'woopb_edit_short_link' );
		...
		if ( count( $posts ) ) {
			...
			wp_send_json_success( site_url( "?woopbUrl={$post->post_title}" ) );
		}
		...
		$meta_id = update_post_meta( $post_id, '_product_array', $meta_data );
		...
		if ( $meta_id ) {
			wp_send_json_success( site_url( "?woopbUrl={$code}" ) );
		}
