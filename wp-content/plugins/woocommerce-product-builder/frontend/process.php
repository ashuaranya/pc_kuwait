		...
		if ( wp_verify_nonce( $_POST['_nonce'], '_woopb_add_to_cart' ) ) {
			...
			if ( $product_id ) {
				...
				if ( ! $enable_multi_select ) {
					...
					$clear_filter = $this->settings->get_option( 'clear_filter' );
					if ( $clear_filter ) {
						$remove = array();
						if ( ! empty( $_GET ) ) {
			...
			if ( count( $steps ) ) {
				foreach ( $steps as $step ) {
					foreach ( $step as $product_id => $detail ) {
						$variation = ! empty( $detail['attributes'] ) ? $detail['attributes'] : array();
	...
	public function share_link_callback() {
		...
		if ( isset( $_GET['woopbUrl'] ) ) {
			...
			if ( empty( $_GET['woopb_edit_short_link'] ) ) {
				$clicked = get_post_meta( $post_id, '_clicked', true );
				update_post_meta( $post_id, '_clicked', $clicked + 1 );
			}

			$data = get_post_meta( $post_id, '_product_array', true );
			if ( isset( $data['page_id'], $data['data'] ) ) {
				wc()->session->set_customer_session_cookie( true );
				wc()->session->set( 'woopb_' . $data['page_id'], $data['data'] );
