...
class VI_WPRODUCTBUILDER_FrontEnd_Step {
	...
	public function share_popup_form() {
		...
		if ( isset( $wp_query->query_vars['woopb_preview'] ) ) {
			wc_get_template( 'content-product-builder-preview-popup.php', array(  ), '', VI_WPRODUCTBUILDER_TEMPLATES );
		}
	}
...
//		$post_id = $post->ID;
		...
		<div class="woopb-steps woopb-steps-top">
			<?php if ( count( $step_titles ) ) {
				...
				?>
                <div class="woopb-step-heading <?php echo $current ? 'woopb-step-active woopb-step-heading-active' : ''; ?>">
                    <a href="<?php echo add_query_arg( $arg, get_the_permalink() ) ?>" class="woopb-step-link">
						<?php echo '<img src="'.$step_title.'" height="50" width="50"/>'?>
                    </a>
                </div>
			<?php } 
		}	?>
        ...
        <div class="woopb-steps">
			<?php if ( count( $step_titles ) ) {
				...
				foreach ( $step_titles as $k => $step_title ) {
                            ...
                            <div class="woopb-step-products-added">
								<?php foreach ( $products_added as $p_id => $value ) { ?>
                                    <div class="woopb-step-products-added-wrapper">
										...
										$sub_price     = $product->get_price() * $value['quantity'];
										$total_price   += $sub_price;
										$attr          = $product->is_type( 'variation' ) ? ' (' . implode( ', ', $value['attributes'] ) . ') ' : '';
                                        ...
                                        <div class="woopb-step-product-added">
											<?php printf( '<span class="woopb-step-product-added-title"><a href="%s" target="_blank">%s</a></span>',
												$product->get_permalink(), $product->get_title() . $attr ); ?>
					...
					<?php } ?>
				<?php } ?>
			<div class="woopb-step-total-remove">
						...
						if($_SESSION['isAssembled'] == 'checked'){
							...
							foreach ($sql as $value) {
								...
								if($total_price >= $value->min_amount && $total_price <= $value->max_amount){
									...
									$total_price = apply_filters('get_assemble_price', $total_price, $charges);
								}
							}	
						}
						echo esc_html__( 'Total:', 'woocommerce-product-builder' ) . ' ' . wc_price( $total_price ); 
					?>
                </div>
				<?php if ( $this->get_data( $post_id, 'remove_all_button' ) ) { ?>
                    <div class="woopb-step woopb-step-remove">
                        <a class="woopb-step-product-added-remove-all" href="<?php
	...
	public function init_scripts() {
		...
		if ( WP_DEBUG ) {
			...
			wp_register_script( 'woocommerce-product-builder', VI_WPRODUCTBUILDER_JS . 'woocommerce-product-builder.min.js', array(
				'jquery',
				'wc-single-product'
			),
				VI_WPRODUCTBUILDER_VERSION );
